...
// q: What's the duration of each frame defined in this file? a: 20ms q: How do you know? a: 48000 * 0.02 = 960
...
opus::Encoder::Encoder(opus_int32 sample_rate, int num_channels,
  ...
  if (!valid()) {
    std::cerr << "Could not construct encoder. Error: " << ErrorToString(error) << std::endl;
    return;
  }
  if (expected_loss_percent > 0) {
    std::cerr << "Enabling FEC in the encoder." << std::endl;
...
std::vector<std::vector<unsigned char>> opus::Encoder::Encode(
  ...
  if (data_length % frame_length != 0u) {
    std::cerr << "PCM samples contain an incomplete frame. Ignoring the "
...
std::vector<unsigned char> opus::Encoder::EncodeFrame(
  ...
  if (num_bytes < 0) {
    std::cerr << "Encode error: " << opus::ErrorToString(num_bytes) << std::endl;
    return {};
  }
  ...
  std::cout << "Encoding Frame time: " << encodeDuration.count() << " us\n";
...
std::vector<opus_int16> opus::Decoder::Decode(
  ...
  for (const auto& enc : packets) {
    ...
    std::cout << "Decoding Frame time: " << decodeDuration.count() << " us\n";
...
std::vector<opus_int16> opus::Decoder::Decode(
  ...
  if (num_samples < 0) {
    std::cerr << "Decode error: " << opus::ErrorToString(num_samples) << std::endl;
    return {};
  }
